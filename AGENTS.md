...
- CI package path filters, matrix package ids, and root scripts `format:check:*` / `lint:*` / `test:*` / `check:*` in `.github/workflows/ci.yml` and root `package.json` MUST stay synchronized when a workspace package is added, renamed, or moved; the `store` filter sync rule lives in `packages/harness/src/agent-session/store/AGENTS.md`.
...
- OpenAPI spec under `fern/openapi/openapi.json` is autogenerated in CI and MUST NOT be edited manually
