...
<Note>
  A turn can also end paused when the agent asks a clarifying question ([`tool.response_required`](/create-agent/ask-user-questions))
...
| `user.tool_approval` | An [approval decision](/create-agent/tool-approval) for a paused tool call.                 |
| `user.tool_response` | An [answer](/create-agent/ask-user-questions) for a paused client-side tool call.           |
...
The same pattern applies to [`tool.response_required`](/create-agent/ask-user-questions) (resume with `user.tool_response`) and [`mcp.auth_required`](/mcp-servers#in-chat-authentication) (resume after the user completes OAuth).
