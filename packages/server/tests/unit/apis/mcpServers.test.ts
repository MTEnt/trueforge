...
describe('mcp-server PUT under the route transaction', () => {
  ...
  it('commits the upsert when the route transaction succeeds', async () => {
    ...
    expect(response.status).toBe(200);
    const stored = await store.getServer({ tenant_id: TENANT_ID, name: putBody.name });
  ...
  it('rolls back the upsert when the route transaction fails', async () => {
    ...
    await expect(store.getServer({ tenant_id: TENANT_ID, name: putBody.name })).resolves.toBeUndefined();
  });
});
