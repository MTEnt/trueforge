...
console.log('session:', session.id, 'seed turn:', created.id, pending.state.status);

for await (const turn of await client.sessions.listTurns(session.id)) {
  if (turn.state.status === 'running') continue;
  for await (const event of await client.sessions.listTurnEvents(session.id, turn.id, { order: 'asc' })) {
